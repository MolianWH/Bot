...
from DDAIRDesk.tools.dy_pb2 import PushFrame
from DDAIRDesk.tools.dy_pb2 import Response
from DDAIRDesk.tools.dy_pb2 import MatchAgainstScoreMessage
from DDAIRDesk.tools.dy_pb2 import LikeMessage
from DDAIRDesk.tools.dy_pb2 import MemberMessage
from DDAIRDesk.tools.dy_pb2 import GiftMessage
from DDAIRDesk.tools.dy_pb2 import ChatMessage
from DDAIRDesk.tools.dy_pb2 import SocialMessage
from DDAIRDesk.tools.dy_pb2 import RoomUserSeqMessage
from DDAIRDesk.tools.dy_pb2 import UpdateFanTicketMessage
from DDAIRDesk.tools.dy_pb2 import CommonTextMessage
from DDAIRDesk.tools.read_yaml import read_yaml

# dd_sock = DDSocket()
# socket_type = dd_sock.socket_type
# sock_pairs = dd_sock.set_sock(set_pairs=[{"sock_type": socket_type.TCP_CLIENT, "addr": ('127.0.0.1', 8888)}])
...
class DDUnPack:
    def __init__(self):
        yaml_path = "../config/local/DouYinUnPack.yaml"
        # 读取remote配置
        try:
            self.config = read_yaml(yaml_path, 'DDUnPack')
            self.live_room_url = self.config['live_room_url']  # 本机ip，负责给UE发送chat对话信息
    ...
    def start(self):
        ...
        self.parseLiveRoomUrl(self.live_room_url)
        logger.info("抖音抓包已启动成功。")
    ...
    def unPackWebcastChatMessage(self, data):
        ...
        msg = data['content']
        self.set_msg(msg={"content": msg}, interval=self.interval)
        logger.info('直播间弹幕消息] [房间Id：' + liveRoomId + '] ｜ ' + msg)
        # dd_sock.tcp_client.set_data(bytes(data['content'], encoding='utf8'))
        return data
    ...
    def unPackWebcastGiftMessage(self, data):
        ...
        log = json.dumps(data, ensure_ascii=False)
        msg = {"giftNickName": data["user"]["nickName"], "giftName": data["gift"]["name"]}
        self.set_msg(msg, 2 * self.interval)
    ...
    def unPackWebcastMemberMessage(self, data):
        ...
        msg = {"nickName": data["user"]["nickName"]}
        self.set_msg(msg, 3 * self.interval)
        return data
    ...
    def unPackWebcastLikeMessage(self, data):
        ...
        log = json.dumps(data, ensure_ascii=False)
        msg = {"likeNickName": data["user"]["nickName"]}
        self.set_msg(msg, 3 * self.interval)
    ...
    def wssServerStart(self, roomId):
        global liveRoomId
        liveRoomId = roomId
    ...
    def parseLiveRoomUrl(self, url):
        ...
        ttwid = data['ttwid']
        res = res.content
        self.wssServerStart(str(res[res.find(b'roomId') + 11:res.find(b'roomId') + 30])[2:-1])
        # res = res.text
        ...
        # self.wssServerStart(liveRoomId)
    ...
    def set_msg(self, msg, interval):
        """
        时间间隔：
            普通消息：self.interval
            进入直播间：3*self.interval
            送礼物：2*self.interval
            点赞：3*self.interval
        Args:
            msg:
